...
export function parseFormPayload(d) {
  ...
  const link_labs = paciente_nome_raw
    ? `http://localhost:3000/api/buscar?nome=${encodeURIComponent(paciente_nome_raw)}`
    : null;
  ...
  return {
    ...
    peso_nascimento:        toN(d.peso_neo),
    acesso_vascular_neo:    d.acesso_neo    || [],
    insuficiencia_renal:    d.ir            || [],
    clcr:                   toN(d.clcr),
    peso:                   toN(d.peso),
    altura:                 toN(d.altura),
    ...
    oxacilina_associacao:   toB(d.oxacilina),
    crm:                    d.crm           || null,
